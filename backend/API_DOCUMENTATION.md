...
- `direction` - фильтр по направлению (Creative, IT, Design, E-commerce, HoReCa, Architecture & Design, Production, Marketing, Sales & Business Development, Finance & Accounting, HR & People, Operations & Logistics, Education, Legal & Compliance)
...
## AI-индекс устойчивости профессии

### Получение карточки индекса по профилю

**Endpoint:** `GET /api/career/ai-risk`

**Права доступа:** Авторизованный пользователь (с созданным профилем)

**Описание:** Возвращает запись из справочника `ai_risk_index` по паре `direction` + `level` текущего профиля (rule-based, без ML). Справочник заполняется скриптом `npm run seed`.
...
```json
{
  "profile": { "direction": "IT", "level": "Senior" },
  "id": "...",
  "riskLevel": "low",
  "riskScore": 20,
  "riskDescription": "Низкий риск (20)...",
...
**Возможные ошибки:**
- `400` — профиль не найден
- `404` — для пары направление+уровень нет строки в справочнике (не выполнен seed)
- `401` — не авторизован

---

## Хаб «Меняю профессию» (блок 5 ТЗ)

### Контент для трека смены профессии

**Endpoint:** `GET /api/career/career-change-hub`

**Права доступа:** Авторизованный пользователь с профилем, у которого `careerChangeTrackActive === true`.

**Описание:** Возвращает подборку материалов из `learning_resources` с тегом `career_change` (плюс модификаторы `from_scratch`, `internship_kz`, `internship_abroad`, `success_story`), статические ссылки на госпрограммы РК и шаблонные истории успеха. Если трек выключен — `403`.
...
**Пример успешного ответа (200):** объект с полями `tone`, `intro`, `track` (данные из профиля), `governmentPrograms`, `learningFromScratch`, `learningGeneral`, `internshipsKz`, `internshipsAbroad`, `successStories`, `successStoryResources`.

**Возможные ошибки:**
- `400` — профиль не найден
- `403` — трек «Меняю профессию» не включён в профиле
- `401` — не авторизован

**Поля профиля (POST/PUT `/api/profile`, опционально):**
- `careerChangeTrackActive` (boolean)
- при `true`: `careerChangeCurrentField`, `careerChangeTargetDirection`, `careerChangeMotivation`, `careerChangeTimeline`; опционально `careerChangeAgeRange`
