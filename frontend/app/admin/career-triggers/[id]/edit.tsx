...
export default function AdminEditCareerTriggerScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const { t } = useTranslation('common');
  const router = useRouter();
  const [initialValues, setInitialValues] = useState<CareerTriggerPayload>();
  ...
  async function handleSubmit(payload: CareerTriggerPayload) {
    ...
    setError(null);
    try {
      await updateCareerTrigger(id, payload);
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-gray-900">
      <AdminHeader
        title={t('adminCareerTriggers.editTitle')}
