...
export default function AdminEditCompanyScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const { t } = useTranslation('common');
  const router = useRouter();
  const [initialValues, setInitialValues] = useState<CompanyPayload>();
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-gray-900">
      <AdminHeader title={t('adminCompanies.editTitle')} />
      <CompanyForm
